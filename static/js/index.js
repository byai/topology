...
var shouldAutoLayout=function shouldAutoLayout(nodes){if(nodes.length===0){return false;}return!nodes.find(function(item){return!!item.position;});};/** 获取相对画布的坐标 */var computeCanvasPo=function computeCanvasPo(position,$wrapper){// 当窗口有滚动时，需加上窗口的滚动
var rect=$wrapper.getBoundingClientRect();return{x:position.x+$wrapper.scrollLeft+window.pageXOffset-rect.left,y:position.y+$wrapper.scrollTop+window.pageYOffset-rect.top};};/** 计算连接线路径 */var computeLinePath=function/* istanbul ignore next */computeLinePath(start,end){var lineOffsetY=arguments.length>2&&arguments[2]!==undefined?arguments[2]:0;// const svgPath = path();
// // 直线绘制方式（代码暂时保留）
 ...
 * 滚动 Y 轴距离顶部距离
 */var computeContentPostionY=function computeContentPostionY(nodes){if(!computeMaxAndMin(nodes))return null;var _computeMaxAndMin2=computeMaxAndMin(nodes),minX=_computeMaxAndMin2.minX,maxX=_computeMaxAndMin2.maxX,minY=_computeMaxAndMin2.minY;return{x:(minX+maxX)/2,y:minY};};/** 计算节点连接处 */var computeNodeInputPo=function computeNodeInputPo(node){var $node=document.getElementById("dom-map-".concat(node.id));if(!$node){return null;}var nodeSize=getNodeSize($node);// eslint-disable-next-line no-param-reassign
node.position=node.position||{x:0,y:0};var po={x:node.position.x+nodeSize.width/2,y:node.position.y};if(Number.isNaN(po.x)||Number.isNaN(po.y)){return null;}return po;};/** 计算鼠标相对画布位置 */var computeMouseClientToCanvas=function computeMouseClientToCanvas(clientX,clientY,$wrapper){var rect=$wrapper.getBoundingClientRect();var dX=clientX-rect.left;var dY=clientY-rect.top;return{x:$wrapper.scrollLeft+dX,y:$wrapper.scrollTop+dY};};var utils_createHashFromObjectArray=function createHashFromObjectArray(arr,field){return arr.reduce(function(pre,cur){var key=typeof field==='string'?cur[field]:field(cur);return _objectSpread({},pre,_defineProperty({},key,cur));},{});};/**
...
var config={transition:'.2s',autoLayout:{/** 节点间水平间距 */horizontalSpacing:20,/** 节点间垂直间距 */verticalSpacing:80},canvas:{/** 画布宽度 */width:40000,/** 画布高度 */height:40000},line:{/** 线条宽度 */strokeWidth:1,strokeLargeWidth:2,/** 箭头宽度 */triangleWidth:10,/** 触发区域宽度 */triggerWidth:20}};/* harmony default export */ var lib_config = (config);
...
/* eslint-disable no-debugger */ /* eslint-disable no-param-reassign */function computeClientRect(node){node.clientRect=getNodeSize(node.id);return node;}function getTreeNode(data){if(data.nodes.length<=0){return null;}// 不满足树的条件
if(!onlyOneParent(data)||!tree_onlyOneRoot(data)){return null;}// 将数组转为树结构，并获取节点的dom大小
